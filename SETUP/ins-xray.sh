...
latest_version="$(curl -fsSL https://api.github.com/repos/XTLS/Xray-core/releases/latest | grep 'tag_name' | sed -E 's/.*"v?([^"]+)".*/\1/' | head -n 1)"
...
{
  "log": {
    "access": "/var/log/xray/xhttp-access.log",
...
# enable xray vless xhttp
echo -e "[ ${green}OK${NC} ] Restarting Vless XHTTP"
systemctl daemon-reload
systemctl enable xray@xhttp.service
systemctl start xray@xhttp.service
systemctl restart xray@xhttp.service
