...
purple() { printf '%b' "${purple}${*}${NC}"; }
tyblue() { printf '%b' "${tyblue}${*}${NC}"; }
yellow() { printf '%b' "${orange}${*}${NC}"; }
red() { printf '%b' "${red}${*}${NC}"; }
...
clear
#Install Caddy
echo -e "\e[0;32mINSTALLING CADDY AS REVERSE PROXY FOR PORT 80...\e[0m"
sleep 1
wget https://${Server_URL}/SETUP/setup-caddy.sh && chmod +x setup-caddy.sh && ./setup-caddy.sh
echo -e "\e[0;32mDONE INSTALLING CADDY REVERSE PROXY\e[0m"
echo -e ""
sleep 1
clear
#install ohp-server
...
wget -O quota_check "https://${Server_URL}/BOT/quota_check.sh"
