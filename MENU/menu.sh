...
# // CADDY status
nginx=$( systemctl status caddy | grep Active | awk '{print $3}' | sed 's/(//g' | sed 's/)//g' )
if [[ $nginx == "running" ]]; then
    status_caddy="${GREEN}ON${NC}"
else
    status_caddy="${RED}OFF${NC}"
fi
# // ssh websocket status
nginx=$( systemctl status ws-http | grep Active | awk '{print $3}' | sed 's/(//g' | sed 's/)//g' )
if [[ $nginx == "running" ]]; then
    status_wshttp="${GREEN}ON${NC}"
else
    status_wshttp="${RED}OFF${NC}"
fi
# // xray status
...
echo -e "\e[36m╒════════════════════════════════════════════════════╕\033[0m"
echo -e "     [ XRAY-CORE${NC} : ${status_xray} ]   [ CADDY${NC} : ${status_nginx} ]"
echo -e "     [ NGINX${NC} : ${status_nginx} ]       [ SSH-WS${NC} : ${status_wshttp} ]"
echo -e "\e[36m╘════════════════════════════════════════════════════╛\033[0m"
